...
def patchify_target(
    ...
    patchified_batch = torch.cat(patchified, dim=0).cuda().to(torch.float16)  
    patchified_batch = torch.cat([p.unsqueeze(0) for p in patchified], dim=0).cuda().to(torch.float16)  


    seq_len = patchified_batch.shape[1]

    cp_size = parallel_state.get_context_parallel_world_size()
    cp_rank = parallel_state.get_context_parallel_rank()

    assert seq_len % cp_size == 0, "seq_len must be divisible by cp_size"
    interval = aligned_len // cp_size

    # x.shape = [B, S, D] => CP 拆的是 S（patch 维度）
    ...
    patchified_batch = patchified_batch[:, tp_rank * tp_interval : (tp_rank + 1) * tp_interval, :]
    ...
    with stimer:
        if parallel_state.is_pipeline_first_stage():
            with torch.no_grad():
                ...
                # clip_pixel_values = torch.randn(1, 720, 1248, 3, dtype=dtype, device=device, requires_grad=False)

                # # 多通道 mask 图像：21 个目标，每个 3 通道图像
                # mask_pixel_values = torch.randn(1, 21, 3, 720, 1248, dtype=dtype, device=device, requires_grad=False)

                # # 二值掩码：值为 0 或 1，float 类型
                # mask = torch.randint(0, 2, (1, 21, 1, 720, 1248), dtype=dtype, device=device, requires_grad=False)

                # # 多个目标输入图像：21 个目标的 RGB 图像
                # pixel_values = torch.randn(1, 21, 3, 720, 1248, dtype=dtype, device=device, requires_grad=False)
        ...
        output_tensor = model(noisy_latents, timestep, context, grid_sizes, context_mask, 
                            attn_mask, clip_context, inpaint_latents, sigmas, target)

    # loss_mask = None #no padding
    loss_mask = torch.ones((noisy_latents.size(0), int(grid_sizes[0][0]) * int(grid_sizes[0][1]) * int(grid_sizes[0][2]))).cuda()
...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    ...
    if parallel_state.is_pipeline_first_stage():
        ...
        train_dataloader = torch.utils.data.DataLoader(
            ...
            worker_init_fn=worker_init_fn(args.seed + dp_rank)
        )
