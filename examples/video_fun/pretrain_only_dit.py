...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    """Build the train, validation, and test datasets with fake data.

    Args:
        train_val_test_num_samples : A list of three integers [train, valid, test]
    """
    def make_fake_loader(num_samples):
        # 每个样本是一个 32维向量，标签是一个整数
        device = torch.cuda.current_device()
        x = torch.randn(num_samples, 32, device=device)  # Move to GPU (if available)
        y = torch.randint(0, 10, (num_samples,), device=device)  # Move to GPU (if available
        dataset = TensorDataset(x, y)
        return DataLoader(dataset, batch_size=8, shuffle=True)

    if parallel_state.is_pipeline_first_stage():
        train_loader = make_fake_loader(train_val_test_num_samples[0])
        valid_loader = make_fake_loader(train_val_test_num_samples[1])
        test_loader  = make_fake_loader(train_val_test_num_samples[2])
